...
namespace CarRentalDispatchBot.NlpDispatch
{
    ...
    public class NlpDispatchBot : IBot
    {
        ...
        /// <summary>
        /// Key in the Bot config (.bot file) for the Luis CarRental.
        /// </summary>
        ...
        public NlpDispatchBot(BotServices services)
        {
            ...
            if (!_services.LuisServices.ContainsKey(CarRentalLuisKey))
            {
                throw new System.ArgumentException($"Invalid configuration. Please check your '.bot' file for a Luis service named '{CarRentalLuisKey}'.");
            }
        }
        ...
        private async Task DispatchToTopIntentAsync(ITurnContext context, (string intent, double score)? topIntent, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                case carRentalLuisKey: // todo
                    await DispatchToLuisModelAsync(context, CarRentalLuisKey);
